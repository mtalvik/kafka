...
1..100 | ForEach-Object { "order_id=$($_);status=created;ts=$(Get-Date -Format o)" } | docker compose exec -T kafka kafka-console-producer --bootstrap-server kafka:9092 --topic orders

Write-Host "Reading 10 messages with consumer group demo-group so a committed offset appears" -ForegroundColor Cyan
docker compose exec kafka kafka-console-consumer --bootstrap-server kafka:9092 --topic orders --group demo-group --from-beginning --max-messages 10

Write-Host "Sending another 200 messages. demo-group should now have lag" -ForegroundColor Cyan
101..300 | ForEach-Object { "order_id=$($_);status=created;ts=$(Get-Date -Format o)" } | docker compose exec -T kafka kafka-console-producer --bootstrap-server kafka:9092 --topic orders
