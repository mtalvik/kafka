...
> `transaction.timeout.ms` на уровне брокера/producer, но Streams переопределяет
> его для своего producer: фактическое значение — **10 с**, проверено на
> Kafka 4.3 через `kafka-transactions.sh describe`
> (`TransactionTimeoutMs 10000`). То есть брошенная транзакция держит
